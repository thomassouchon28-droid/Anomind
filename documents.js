/* documents.js — Bandeau d'équipe (nom, points, renommer, changer), classement, liste et lecture des documents, devinette, et formulaire de relevé d'anomalie. Dépend de state.js. */
...
function renderMe(){
  ...
  $("#meChangeBtn").hidden = (S.phase==="playing");
...
function renderBoard(){
  ...
  ranked.forEach(r=>{
    ...
    const absente = S.db && S.claims.length>0 && !holderOf(r.t.id);
    const n = el("div","t"+(r.t.id===S.me?" self":"")+(r.p<=0?" dead":"")+(absente?" away":""));
    n.style.setProperty("--tc", r.t.c);
    const l = el("div","l");
    l.append(el("b",null,teamName(r.t.id)), el("i",null, r.p<=0 ? "hors jeu" : String(r.p)));
    n.append(l);
    if(absente) n.append(el("span","away-tag","déconnectée"));
...
function renderOver(){
  ...
  const ranking = activeTeams().sort((a,b)=> pts(b.id)-pts(a.id) || foundCount(b.id)-foundCount(a.id));
  ...
  if(reason==="elimination"){
    const nb = activeTeams().filter(t=>dead(t.id)).length;
    box.append(el("p",null, nb+" équipes sur "+activeTeams().length+" sont tombées à zéro point. "+teamName(winner.id)+" remporte la partie."+tieNote));
  } else {
...
function renderDocList(){
  ...
  orderedDocs().forEach(d=>{
    ...
    const m = el("div","meta");
    if(ownerOf(d.id)===null) m.append(el("span",null,"Remis à toutes les équipes"));
    else if(ownerOf(d.id)===S.me){ const dot=el("span","owner-dot"); dot.style.setProperty("--tc",team(S.me).c);
...
async function guess(docId, guessId){
  ...
  if(!d || hasDoc(docId, S.me)) return;                 // document déjà accessible
  if(S.guesses.some(g=>g.team===S.me && g.docId===docId && g.guess===guessId)) return;   // équipe déjà tentée
  const right = ownerOf(d.id) === guessId;
  const id = uid();
...
/* Les menus du formulaire sont reconstruits à chaque synchronisation (une
   déclaration, une devinette ou un message d'une autre équipe en déclenche
   une). Reconstruire sans condition remettait le menu sur sa première option
   et faisait donc « sauter » la ligne en cours de saisie. On ne reconstruit
   désormais que si la liste des options a réellement changé, et on rétablit
   alors la sélection en cours. */
function syncSelect(sel, opts, keepValue){
  const sig = opts.map(o=>o.v+"\u0000"+o.t).join("\u0001");
  if(sel.dataset.sig === sig) return;
  const cur = keepValue ? sel.value : null;
  sel.innerHTML="";
  opts.forEach(o=>{ const n=el("option",null,o.t); n.value=o.v; sel.append(n); });
  sel.dataset.sig = sig;
  if(cur && opts.some(o=>o.v===cur)) sel.value = cur;
}
function fillDocSelect(){
  syncSelect($("#aDoc"), myDocs().map(d=>({v:d.id, t:d.code+" — "+d.titre})), true);
  fillLines();
}
function fillLines(){
  const s = $("#aLig");
  const d = docById($("#aDoc").value);
  if(!d){ s.innerHTML=""; s.dataset.sig=""; s.dataset.doc=""; return; }
  // Les références de ligne (L01, L02…) se répètent d'un document à l'autre :
  // on ne conserve la sélection que si l'on est resté sur le même document.
  const sameDoc = s.dataset.doc === d.id;
  const opts = d.lignes.map(l=>{
    const lbl = String(l.c[0]).slice(0,54);
    return {v:l.ref, t: l.ref+" — "+lbl+(String(l.c[0]).length>54?"…":"")+(isFound(d.id,l.ref)?"  (déjà relevée)":"")};
  });
  syncSelect(s, opts, sameDoc);
  s.dataset.doc = d.id;
}
function coolLeft(){
...
async function declare(){
  ...
  if(coolLeft()>0) return;   // règle imposée ici aussi, pas seulement par le bouton
  $("#aSend").disabled = true;
  ...
  const targets = activeTeams().filter(t=>t.id!==S.me && !dead(t.id));
